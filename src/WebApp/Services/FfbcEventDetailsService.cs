...
/// <summary>
/// Fetches enriched event details from the FFBC popup API with caching.
/// </summary>
public partial class FfbcEventDetailsService : IEventDetailsService
{
    ...
    internal static EventDetails? ParseEventDetails(string eventId, string html)
    {
        ...
        // Registration URL from subscription link
        ...
        // Start location and address from place div
        ...
        if (clubNode is not null)
        {
            ...
            // Strip the "Club organisateur :" prefix
    ...
    {
        // Extract maps URL from the directions link
        ...
        // Get the text content, split by <br> tags
        // Structure: "<b>Lieu de départ :</b><br> Venue Name <br> Address <br> <a>..."
        var innerHtml = placeNode.InnerHtml;
        var parts = innerHtml.Split(new[] { "<br>", "<br/>", "<br />" }, StringSplitOptions.RemoveEmptyEntries);

        // Parts after the label, before the maps link
