...
The system SHALL retrieve mountain biking events by sending a POST request to the FFBC calendar API endpoint with the configured province, challenge, year, and date parameters.
...
The system SHALL cache the parsed event list in memory for a configurable duration to avoid redundant remote requests.

#### Scenario: Cached result returned on subsequent calls
- **WHEN** the event store is called and a valid cached result exists
...
- **THEN** the system SHALL make a new HTTP request on the next `GetAll()` call
...
The system SHALL read the POST URL, province, challenge, year, and cache TTL from application configuration rather than hard-coding them.
