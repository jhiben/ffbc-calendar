...
The system SHALL serve a valid RSS 2.0 XML feed at the `/feed.xml` URL path containing upcoming events. The feed SHALL be generated by an Azure Function and served via SWA route rewrite.

#### Scenario: Feed returns valid RSS 2.0 XML
- **WHEN** a client sends a GET request to `/feed.xml`
- **THEN** the SWA SHALL rewrite the request to `/api/feed.xml`, and the Azure Function SHALL respond with status 200, content type `application/rss+xml`, and a body that is valid RSS 2.0 XML with `<rss version="2.0">` root element
...
- **WHEN** an event is mapped to an RSS item
- **THEN** the `<link>` SHALL be an absolute URL to the event's detail page (`/event?date=YYYY-MM-DD&title=...`)
